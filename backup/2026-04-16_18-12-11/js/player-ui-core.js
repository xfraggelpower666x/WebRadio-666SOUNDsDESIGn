...
// ERSTELLT: 2026-04-16
// GEÄNDERT: 2026-04-16
// ZWECK: Gemeinsame Frontend-Logik für externen Standard-Player und internen Fallback-Player.
// ÄNDERUNG: One-Page-Cyber-Layout, Health-/Source-/Player-Lampen, Bildlogik und zentrale UI-Konfiguration ergänzt.
// ==========================================
...
export function initPlayer({ streamConfig, uiConfig, mode = 'external', assetPrefix = '.' }) {
  ...
  if (isiOS && elements.volumeHint) elements.volumeHint.textContent = 'Lautstärke über iPhone-Tasten regeln';
      ...
      if (!state.meterAnim) {
        const tick = () => {
          if (state.analyser && state.analyserData) {
            ...
            let left = 0;
            let right = 0;
            for (let i = 0; i < half; i++) left += state.analyserData[i];
            for (let i = half; i < state.analyserData.length; i++) right += state.analyserData[i];
