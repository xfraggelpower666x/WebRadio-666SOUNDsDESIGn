...
function setSource(isFallback){usingFallback=isFallback;if(sourceLabel)sourceLabel.textContent=isFallback?"BACK":"MAIN";if(fallbackText)fallbackText.textContent=isFallback?"Manual":"Manual";setLamp(sourceLamp,isFallback?"lamp-green":"lamp-cyan");updateSourceButtons()}
...
async function safePlay(){userStopped=false;try{if(manualSource==="backup"){await tryPlayFallback()}else{await tryPlayPrimary()}setStatus("Playing");setLamp(audioLamp,"lamp-green");startMetadataLoop();return true}catch(e){setStatus("Audio Error");setLamp(audioLamp,"lamp-red");return false}}
async function switchSource(nextSource){if(switchingSource||manualSource===nextSource)return;switchingSource=true;userStopped=false;manualSource=nextSource;updateSourceButtons();setStatus(nextSource==="backup"?"Switching to BACK":"Switching to MAIN");try{if(nextSource==="backup"){await tryPlayFallback()}else{await tryPlayPrimary()}setStatus("Playing");setLamp(audioLamp,"lamp-green");startMetadataLoop()}catch(err){setStatus("Audio Error");setLamp(audioLamp,"lamp-red")}finally{switchingSource=false}}
...
audio?.addEventListener("playing",()=>{if(userStopped||switchingSource)return;setStatus("Playing");setLamp(audioLamp,"lamp-green")});
...
setLamp(metaLamp,"lamp-red");setLamp(audioLamp,"lamp-red");setLamp(sourceLamp,"lamp-cyan");updateSourceButtons();setSource(false);setMetadataStatus("Loading...");fetchMetadata();`;
