(() => {
  ...
  const meterLCanvas = () => document.getElementById("meterLCanvas");
  const meterRCanvas = () => document.getElementById("meterRCanvas");
  ...
  function drawMobileEq(data, avg) {
    const c = eqCanvas();
    if (!c) return;
    ...
    ctx.globalAlpha = 1;
    ctx.strokeStyle = "rgba(85,232,255,.25)";
    ctx.lineWidth = 2;
    ctx.strokeRect(0, 0, w, h);
  }

  function drawDesktopEq(data, avg) {
    const c = eqCanvas();
    if (!c) return;
    const ctx = c.getContext("2d");
    const rect = c.getBoundingClientRect();
  ...
  function frame() {
    ...
    if (DeviceMode.mode === "iphone") drawMobileEq(data, avg);
    else drawDesktopEq(data, avg);

    drawMeter(meterLCanvas(), avg);
    drawMeter(meterRCanvas(), avg);

    document.documentElement.style.setProperty("--reactive-mix", avg.toFixed(3));
    document.documentElement.style.setProperty("--reactive-peak", Math.min(1, avg * 1.3).toFixed(3));
