/* 666SOUNDsDESIGn Radio — Central LYVRA Boot Screen v1.0.0
...
(function installS666CentralBootScreen(global){
  ...
  async function mount(){
    ...
    const startButton=root.querySelector('#s666boot-start');
    if(startButton) startButton.addEventListener('click',()=>{
      const legacyButton=document.getElementById('bootButton');
      if(legacyButton&&typeof legacyButton.click==='function') legacyButton.click();
      hide('user-start');
    });
    return root;
  }
  function nodes(){return root?{bar:root.querySelector('#s666boot-bar'),percent:root.querySelector('#s666boot-percent'),track:root.querySelector('#s666boot-track'),status:root.querySelector('#s666boot-status'),phase:root.querySelector('#s666boot-phase'),steps:[...root.querySelectorAll('.s666boot-step')],start:root.querySelector('#s666boot-start')}:{}}
  ...
  function complete(reason='complete'){
    if(!root) return;cancelAnimationFrame(raf);setProgress(100);const n=nodes();if(n.status)n.status.textContent='BOOT COMPLETE';if(n.phase)n.phase.textContent='666SOUNDsDESIGn READY';n.steps?.forEach(step=>{step.classList.remove('active');step.classList.add('done');});
    const legacyButton=document.getElementById('bootButton');if(legacyButton&&n.start){n.start.hidden=false;root.dataset.state='awaiting-user';}else{global.setTimeout(()=>hide(reason),180);}
  }
