...
/* v36.3.2 MOBILE VISIBLE STATE HOTFIX
   - Footer version hard-corrected.
   - Empty EQ panel gets visible bars.
   - Now Playing text is prevented from starting clipped.
   - Small mobile logo gets a practical minimum size.
*/
(function(){
  "use strict";
  var VERSION="v36.3.2";
  ...
  function fixMobileVersion(){
    qsa("#mffApp .smfp-version-badge,#mffApp .mff-version-inline").forEach(function(n){
      n.textContent = "WebRadio 666SOUNDsDESIGn " + VERSION;
      n.setAttribute("data-mobile-version-fixed","v36.3.2");
      n.style.visibility = "visible";
      n.style.opacity = "1";
    });
  }

  function ensureEqBars(){
    var eq = qs("#mffEqBars");
    if(!eq) return;
    eq.setAttribute("data-mobile-eq-visible-fix","v36.3.2");
    eq.style.display = "grid";
    eq.style.visibility = "visible";
    eq.style.opacity = "1";
    eq.style.minHeight = "86px";
    ...
    if(bars.length < 24){
      ...
      for(var i=0;i<40;i++){
        ...
        b.style.setProperty("--mff-eq-h", String(18 + ((i*13)%62)) + "%");
        b.style.height = "var(--mff-eq-h)";
  ...
  function fixNowPlayingClip(){
    var title = qs("#mffTitle") || qs("#mffTicker") || qs("#mffNowTitle") || qs("#mffApp .mff-title") || qs("#mffApp .mff-ticker");
    if(title){
      title.setAttribute("data-mobile-title-visible-fix","v36.3.2");
    ...
    qsa("#mffApp .mff-now strong,#mffApp .mff-now-title,#mffApp [data-now-title]").forEach(function(n){
  ...
  function fixLogoSize(){
    var logo = qs("#mffApp img[src*='phase10-new-header-logo'], #mffApp .mff-logo img, #mffApp .phase10-clean-header-logo img");
    if(logo){
      logo.setAttribute("data-mobile-logo-size-fix","v36.3.2");
      logo.style.width = "min(56vw, 260px)";
