...
(function(){
  ...
  async function triggerSkipNow(){if(!await requireAuth())return;const pw=window.prompt("Admin-Passwort optional. Leer lassen = nur Auth Worker.","");const headers={"content-type":"application/json"};if(pw)headers["x-admin-password"]=pw;$("fp-admin-auth-output").textContent=JSON.stringify(await fetchJson("/api/admin/skip",{method:"POST",headers,body:JSON.stringify({source:"admin-overlay-skip"})}),null,2)}
