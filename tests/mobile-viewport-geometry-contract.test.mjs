...
test('main iPhone shell stays geometrically between the left and right side meters', async () => {
  const css = await read('css/mobile-patches.css');
  assert.match(css, /S666_IPHONE_LAYOUT_APPRETURN_V4 — final mobile geometry owner/);
  ...
  assert.doesNotMatch(css, /S666_IPHONE_LAYOUT_APPRETURN_V4[\s\S]*?width:calc\(100vw - 42px\)!important/);
  assert.doesNotMatch(css, /S666_IPHONE_LAYOUT_APPRETURN_V4[\s\S]*?width:calc\(100vw - 38px\)!important/);
