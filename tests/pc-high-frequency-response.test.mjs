...
test('right-side bands use real proportional compensation without synthetic motion', async () => {
  const eq = await read('js/equalizer.js');
  ...
  assert.match(eq, /const adaptiveResponse = Math\.pow\(relative, 0\.82\) \* 0\.14 \* localGate/);
  assert.match(eq, /const attack = 0\.76 \+ position \* 0\.12/);
  assert.match(eq, /const release = 0\.15 \+ position \* 0\.03/);
  assert.match(eq, /clamp\(spectralResponse \* 0\.86 \+ adaptiveResponse, 0\.012, 1\)/);
