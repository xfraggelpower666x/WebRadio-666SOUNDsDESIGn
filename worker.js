...
const audio=document.getElementById("radio");audio?.setAttribute("playsinline","");audio?.setAttribute("webkit-playsinline","");
...
bootButton?.addEventListener("click",async()=>{if(booted)return;booted=true;bootButton.disabled=true;setStatus("Starting");let started=false;try{started=await safePlay()}catch(e){}await runBootSequence();overlay?.classList.add("hidden");if(!started){await safePlay()}});
